...
const ProcessViewer = ({ steps, title, description, badge }) => {
  ...
  const changeStep = useCallback((newStep) => {
    ...
    setTimeout(() => {
      ...
      setAutoPlay(false);           
      // Reset animation after step change
      setTimeout(() => {
        setAnimation(true);
      }, 100);
    }, 150);
    ...
    if (autoPlay) {
      interval = setInterval(() => {
        setActiveStep(prevStep => {
          const nextStep = prevStep === steps.length - 1 ? 0 : prevStep + 1;
          // Trigger the same transition logic as manual navigation
          setAnimation(false);
          setImageLoaded(false);
  ...
  return (
    <Box sx={{ mb: 10 }}>
      ...
      <Box sx={{ position: 'relative' }}>
        ...
        {/* Main content with improved fade transition */}
        <Fade in={animation && imageLoaded} timeout={400}>
          <div>
            <Box sx={{ p: 2 }}>
